...
function LA_step!(fcell::UnitCell, interaction::Interaction, barostat::Barostat, n::Int, dt::Float64, T::Float64; gamma0::Float64=1.0, gammav::Float64=1.0)
    ...
    sigma_v = sqrt(kb * T * gammav * dt)
    sigma_0 = sqrt(kb * T * gamma0 * dt)
    sigma_ddt_0 = sqrt(kb * T * gamma0 * ddt)

    Rvdt2 = Rv + (Pint - Pe) * dt / 2 + sigma_v * rand(Normal(0, sigma_v)) - gammav * Rv * dt / 2 / barostat.W
    ...
    for j in 1:n-1
        ...
        for i in 1:length(fcell.atoms)
            if j == 1
                pl[2*n+1, i] = pl[1, i] + (dt/2) * fl[i] + rand(Normal(0, sigma_0), 3) - (gamma0 * dt / 2 / mi) * pl[1, i]
            end
            pl[2*j, i] = pl[2*j-1, i] + (ddt/2) * fl[i] + rand(Normal(0, sigma_ddt_0), 3) - (gamma0 * ddt / 2 / mi) * pl[j, i]
        ...
        for i in 1:length(fcell.atoms)
            pl[2*j+1, i] = pl[2*j, i] + ddt/2 * fl[i] + rand(Normal(0, sigma_ddt_0), 3) - (gamma0 * ddt / 2 / mi) * pl[2*j, i]
        end
    end
    ...
    for i in 1:length(fcell.atoms)
        ...
        pl[2*n+1, i] = pl[n+1, i] + (dt/2) * fi - (gamma0 * dt / 2 / mi) * pl[n+1, i] + rand(Normal(0, sigma_v), 3)
    end
    ...
    Pint = pressure_int(fcell, interaction)
    Rvdt = Rvdt2 + (Pint - Pe) * (dt / 2) + rand(Normal(0, sigma_v)) - gammav * Rvdt2 * dt / 2 / barostat.W
    barostat.Pv = Rvdt
...
for i in 1:100000
    LA_step!(fcell,interaction,barostat,4,0.001,1.0,gamma0=0.5,gammav=0.1)
    if i%100==0
