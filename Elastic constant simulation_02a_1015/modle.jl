...
"""
function cell_energyij0(cell::UnitCell, interaction::Interaction, i::Int, j::Int; ifnormalize::Bool=false)
    cutoff = interaction.cutoff
    atomi = cell.atoms[i]
    atomj = cell.atoms[j]
    bd=abs.(atomj.bound)
...
:param tol: 误差，默认为 1e-3 用于判断div 0错误
"""
function cell_energy0(cell::UnitCell,interaction::Interaction;ifnormalize::Bool=false)
    a,b,c=cell.copy
    # if interaction.cutoff>minimum((cell.lattice_vectors*Vector([a,b,c])))
    #     lv=cell.lattice_vectors*Vector([a,b,c])
    #     cutoff=interaction.cutoff
