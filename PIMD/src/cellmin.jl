...
function BMfit(vl::Vector{Float64},el::Vector{Float64},cpcell::UnitCell,p0::Vector{Float64}=[1200.0, -10.0, 4.0, -1200.0])
    ...
    """
    function gradientDescent!(cell::UnitCell,interaction::AbstractInteraction;ap::Float64=0.01,tol=1e-8,maxiter=1000,checktime=10)
        El=Vector{Float64}([])
        converge=false
        for s in 1:maxiter
    ...
    gradient step
    """
    function gradientStep!(cell::UnitCell,interaction::AbstractInteraction;ap::Float64=0.01)
    ...
    only for cubic cell
    """
    function minimizeEnergy!(cell::UnitCell,interaction::AbstractInteraction;rg::Vector{Float64}=[1.0,10.0],n::Int=1000)
        El=Vector{Float64}([])
    ...
    """
    function minimizeEnergyWithGradient!(cell::UnitCell,interaction::AbstractInteraction;rg::Vector{Float64}=[1.0,10.0],n::Int=1000,maxiter::Int=100,tol::Float64
        =1e-6)
        El=Vector{Float64}([])
        cl = range(rg[1], stop=rg[2], length=n)
        for lt in cl
    ...
    Monte Carlo
    """
    function MonteCarlo!(cell::UnitCell,interaction::AbstractInteraction,ap=0.001,maxstep=100)
